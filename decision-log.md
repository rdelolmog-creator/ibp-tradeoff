...
CAVEAT: `step05b_scoring_and_portfolio_impact.ipynb` has not yet been run by the reviewer in Colab. Every number behind D-031–D-033 (0.657 correlation, 30.8%/24.7% concentration, etc.) is currently sandbox-verified only, matching the same standard applied to earlier steps before their own Colab confirmation. Step 5 should be treated as substantively, not formally, closed until that run happens and its output is reviewed here.
