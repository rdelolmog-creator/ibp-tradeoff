...
**D-084 · Dashboard split into CFO commentary and a collapsed Method & Limitations section; SKU-level planner parameter table added**
DECISION: dashboard commentary split into CFO-facing decisions and a collapsed method-and-limitations section, because methodological caveats necessary for defensibility were displacing the decision content in the primary reading path. Per-line REC text reduced to the specified 3-sentence CFO register (L2: "Act here. €641.5k, 12.6%..."; L1/L3/L4: "Hold." with saving and %); portfolio and service lines added above it. Every caveat previously embedded in REC text (decomposition-ordering artefacts on L1/L4, L2's min-run floor and bias degeneracy, "best evaluated policy" not being continuous, range-dependent lever ranking, conversion excluding fixed absorption, D-037's pending status, overhang decay) moved verbatim into a `<details>` element, collapsed by default, plus the additionally-requested items (synthetic data, no lead time, monthly ageing granularity, cover-vs-demand measurement basis, service frozen with pre-D-079 evidence only).

One factual correction caught before writing: the instruction's own list stated "cover measured against plan rather than demand." Checked directly against `_excess_units()` in `src/engine.py` — the real implementation measures against `demand_units`, not plan; measuring against plan was the exact circularity bug found and fixed at D-048. Used the correct, verified fact in the Method section rather than the instruction's stated (incorrect) one.

DECISION: a SKU-level planner parameter table added at the foot of the dashboard, filter-responsive, sortable and copyable, because the decision output has to become planning-system parameters and no artefact currently bridges that step. Columns: sku_id, line, abc_class, category, cover_weeks, service_target (frozen ABC value), min_run_hours, bias_correction, moq_units, min_run_units, shelf_life_days — all either static `sku_master` facts or direct reads of current slider state, needing no grid interpolation. `fill_rate` and closing stock omitted entirely (not shown as placeholders) — not available at SKU level for arbitrary slider positions. Verified: table re-renders correctly on both line switch and slider change (tested by dragging min-run and confirming the table's own min-run column updated); ABC column sorts A→B→C via an explicit order map, not alphabetically; "Copy as TSV" writes header + currently-sorted rows to the clipboard, verified by reading the clipboard back in test.

**D-085 · Recommendation moved to top; inline caveats duplicated in Method & limitations removed**
DECISION: the recommendation block moved above the KPIs and levers, and inline caveats duplicated in Method & limitations removed from the primary reading path, because the decision content was appearing behind roughly six explanatory blocks and no longer read as a review artefact. Final order verified directly against the target: header → line selector/toggle → Recommendation → four KPIs → cost bridge → levers and badges → by-class → by-owner → Method (collapsed) → planner table — confirmed by grepping section-label line numbers post-edit, not assumed. This required moving the Cost bridge section earlier too (previously after the levers), not just the Recommendation block, since the specified target order places it before them.

Six duplicated blocks removed (D-037 banner, argmin-methodology banner, service-evidence paragraph, KPI-explanation paragraph, static bias-degeneracy note, ranking-methodology paragraph) — each confirmed present in Method & limitations before deletion, not assumed. D-079 paragraph compressed to one line; the dynamic min-run flag text shortened, unchanged in behaviour.

**Real bug caught and fixed before shipping:** deleting the static bias-degeneracy `<div>` left an orphaned JS reference (`getElementById('bias-degenerate-flag').classList.toggle(...)`) that threw `Cannot read properties of null` on every `update()` call — this silently aborted the function before it reached `updateRecommendation()`, leaving the recommendation panel showing placeholder dashes despite no visible console error (only caught via a `pageerror` listener, not `console` messages). Fixed by removing the orphaned line; re-verified with zero page errors across all four lines, both presets, and slider interaction.

**D-086 · D-081's candidate-set re-selection formalised as real, tested code and folded into Step 7's notebook**
DECISION: `search_all_lines_three_lever()`'s stored winner was never re-selected in the actual codebase after D-081 found it beaten by feasible points on the dashboard's wider exploratory grid — that correction existed only as one-off analysis scripts run during the conversation, never committed. Running Colab against the previously-pushed `step07_policy_model.ipynb` would have reproduced the retracted numbers (e.g. L2 €4,500,825.65), not what the dashboard displays (€4,466,609.57) — a real, findable mismatch, not a false-alarm gap.

Added `refine_with_candidate_set()` and `best_evaluated_policy()` to `src/policy_model.py`, implementing D-081's exact methodology as permanent code: build a candidate set (winner + every one-at-a-time improvement on a wider exploration grid + every joint combination of those improvements), evaluate each as a real engine call, argmin over evaluated candidates only — never summing one-at-a-time deltas, which would repeat the additive-attribution error Task 2 corrected. `tolerance_eur` (default 1.0) guards against the exact rounding bug D-081 found (2-decimal curve data compared against a full-precision total made a winner appear to beat itself); covered by a dedicated regression test.

Verified end-to-end in sandbox before push: all four lines' refined totals match the dashboard's "Best evaluated policy" figures exactly (L1 €4,007,875.52, L2 €4,466,609.57, L3 €4,284,219.15, L4 €4,083,576.60), each strictly ≤ the original search's winner, all capacity-feasible. 5 new tests added (149/149 total pass). `step07_policy_model.ipynb` updated with the refinement cells, a check cell, and an extended consolidated report — the dashboard and what Colab will now produce are the same numbers, not two things that need reconciling after the fact.
