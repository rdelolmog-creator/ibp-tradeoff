...
class DataIngestor:
    ...
    def _validate_columns(self, table_key: str, frame: pd.DataFrame) -> None:
        ...
        for col in expected:
            spec = self.schema["raw"][table_key]["columns"][col]
            ...
            # A column the schema declares `constant:` must actually be
            # constant, and constant at the declared value. A source system
            # silently switching currency parses fine as a string and would
            # otherwise pass every other check — the exact gap
            # test_currency_drift_raises was written for.
            if "constant" in spec:
                declared = spec["constant"]
                found = set(frame[col].dropna().unique())
                if found - {declared}:
                    raise SchemaViolation(
                        f"{table_key}.{col} is declared constant={declared!r} but "
                        f"the source contains {sorted(found)}. Refusing to "
                        f"proceed: this changes the meaning of every value "
                        f"downstream, not just this column."
                    )
            if "values" in spec:
                allowed = {str(v) for v in spec["values"]}
                found = {str(v) for v in frame[col].dropna().unique()}
                if found - allowed:
                    raise SchemaViolation(
                        f"{table_key}.{col} contains values outside the "
                        f"declared set {sorted(allowed)}: {sorted(found - allowed)}"
                    )
