...
def get_flagged_skus(
    censoring_diagnostics: pd.DataFrame,
    verdict_column: str = "verdict",
    flagged_values: tuple = ("INCONCLUSIVE",),
) -> List[str]:
    """Read the flagged-SKU list directly from Step 5a's own output.

    Replaces a manual copy-paste step: the reviewer previously had to read
    Step 5a's printed verdict table and hand-type the SKU list into Step 5b.
    That doesn't stitch — this does. Whatever verdict values Step 5a's
    notebook produced for a given dataset are read here programmatically,
    so a different dataset with different flagged SKUs (or a different
    verdict bucket) needs no code change, only different input data.
    """
    if verdict_column not in censoring_diagnostics.columns:
        raise ValueError(
            f"'{verdict_column}' not found in censoring_diagnostics columns: "
            f"{list(censoring_diagnostics.columns)}"
        )
    mask = censoring_diagnostics[verdict_column].isin(flagged_values)
    sku_col = "sku_id" if "sku_id" in censoring_diagnostics.columns else censoring_diagnostics.index.name
    if sku_col == "sku_id":
        return censoring_diagnostics.loc[mask, "sku_id"].tolist()
    return censoring_diagnostics.index[mask].tolist()
