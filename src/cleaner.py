...
class DataCleaner:
    ...
    def _recover_nulls(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        recoverable = null_mask & df["stock_open_units"].notna()
        derived = (
        ...
        # C-11 irrecoverable: month 1 with no prior stock. Drop, state the count.
        irrecoverable = df["actual_units"].isna()
        n_drop = int(irrecoverable.sum())
        if n_drop:
            self.dropped.append(
                {"reason": "null volume in first month, stock_open unknown", "rows": n_drop}
            )
