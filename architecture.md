...
**Week 6 Capstone · AI Finance Accelerator · Mastery Level** Source-of-truth document. Loaded as Claude Project knowledge and version-controlled in the repo. *v4 — revised after the Step 6 gate and the Step 7 redefinition. See Revision history at the end.*
...
| 7 | **Optimal-policy model** | SNAKE → Colab | Sweep the engine per SKU for cost-minimising parameters, then learn SKU attributes → optimal policy; **XGBoost vs linear benchmark + SHAP** | src/policy_model.py, optimal_policy.csv, comparison table |
...
### Why Step 7 learns a policy, not a cost *(v4)*

The engine already computes conversion cost causally, from run hours, changeovers, overtime and absorption. A model that learned cost from engine output would be learning the engine — circular, and §9 territory. So Step 7 does not predict cost.

Instead it answers the question the tool exists for: **what is the optimal parameter set for each SKU?** The engine is swept per SKU to find the cost-minimising inventory cover and service target. That sweep IS the answer, and it is deterministic — no ML involved.

ML then earns its place doing something the sweep cannot: fitting SKU **attributes** (category, ABC class, margin, shelf life, volatility, chronic bias, line speed) to those optimal parameters, so a policy can be set for a SKU the engine cannot sweep. The principal case is a **new launch** — no demand history, so Step 5a cannot characterise it and the engine cannot simulate it, but every attribute the policy model needs is known before the first shipment. SHAP then explains *why* a SKU wants high cover in terms a planner recognises.

Stated plainly for the pitch and the limitations slide: **the optimisation is deterministic; machine learning explains and generalises the policy rather than producing it.** Two honest limits — the most predictive features (volatility, chronic bias) are exactly the ones a new launch lacks, so the launch case runs on a weaker feature set than the training data; and a policy rule fitted on 60 SKUs is defensible as method, not as a number.

Note `min_run_hours` is a line decision, not a per-SKU one, so the per-SKU optimum is over inventory cover and service target. Forecast bias correction is a planner-accountability lever, not a per-SKU policy parameter.
...
ibp-tradeoff/
  ...
  src/        ingest.py  cleaner.py  demand.py
              ...
              policy_model.py  engine.py  reporter.py
...
| Model trade-off explained | Step 7 | XGBoost vs linear, SHAP, architecture argument — including why the optimisation is deterministic and ML explains rather than produces the policy |
...
| # | Section | Change | Ref |
|---|---|---|---|
| 1 | §5, step 7 | Step 7 changed from **learning cost response to schedule disruption** to **learning optimal policy from SKU attributes**. A model learning cost from engine output would be learning the engine. The redefined step sweeps the engine per SKU for cost-minimising parameters (deterministic), then fits attributes → optimal parameters so a policy can be set for SKUs the engine cannot sweep — principally new launches with no demand history. Output renamed `src/cost_model.py` → `src/policy_model.py`. | D-064 |
