...
# ---------------------------------------------------------------------------
# Step 6 revision — O-14 and D-058
# ---------------------------------------------------------------------------


def test_safety_stock_does_not_scale_with_bias_correction(engine, base_levers):
    """O-14: the volatility buffer is sized off demand, not the biased plan.

    Safety stock absorbs volatility, not bias. If it moved with the bias
    correction lever, correcting a known over-forecast would strip service
    protection along with the excess cycle stock — and a planner who
    over-forecasts would be rewarded with more cover than one who does not.
    """
    a = engine.run_scenario(
        MVD_LINE, base_levers.replace(forecast_bias_correction=0.0)
    ).sku_month.set_index(["sku_id", "month"])["safety_stock_units"]
    b = engine.run_scenario(
        MVD_LINE, base_levers.replace(forecast_bias_correction=1.0)
    ).sku_month.set_index(["sku_id", "month"])["safety_stock_units"]
    pd.testing.assert_series_equal(a, b, check_names=False)


def test_cycle_stock_still_scales_with_the_plan(engine, base_levers):
    """The other half of O-14: cover is a policy on the planning signal."""
    a = engine.run_scenario(
        MVD_LINE, base_levers.replace(forecast_bias_correction=0.0)
    ).sku_month.set_index(["sku_id", "month"])["target_stock_units"]
    b = engine.run_scenario(
        MVD_LINE, base_levers.replace(forecast_bias_correction=1.0)
    ).sku_month.set_index(["sku_id", "month"])["target_stock_units"]
    assert float((a - b).abs().max()) > 0.0, "target stock ignored the plan entirely"


def test_production_does_not_diverge_from_demand(engine, base_levers):
    """D-058: capacity backlog must be consumed, not compounded.

    Backlog is deferred volume; the request that carries it also satisfies it.
    Without the reset it accumulates every month and production runs away from
    demand. Latent since Step 6 because L3 never breaches capacity, so this
    asserts the invariant at every lever setting rather than only the default.
    """
    for mr in (2.0, 9.0, 20.0):
        res = engine.run_scenario(MVD_LINE, base_levers.replace(min_run_hours=mr))
        demand = float(res.sku_month["demand_units"].sum())
        produced = float(res.sku_month["production_units"].sum())
        assert produced < 2.0 * demand, (
